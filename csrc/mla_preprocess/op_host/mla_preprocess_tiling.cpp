...
constexpr int64_t MAX_TOKEN_NUM = 1024;
constexpr uint64_t GENERIC_KERNEL_TILING_KEY = 0;

struct MlaPreprocessCompileInfo {};

void PopulateRegisteredTilingData(const MlaTilingData &source, MlaPreprocessTilingData &registered)
{
#define COPY_FIELD(fieldName) registered.set_##fieldName(source.fieldName)
#define COPY_MATMUL(prefix, matmul)                                                                                   \
    registered.set_##prefix##NumBatch((matmul).numBatch);                                                             \
    registered.set_##prefix##M((matmul).m);                                                                           \
    registered.set_##prefix##K((matmul).k);                                                                           \
    registered.set_##prefix##N((matmul).n);                                                                           \
    registered.set_##prefix##M0((matmul).m0);                                                                         \
    registered.set_##prefix##K0((matmul).k0);                                                                         \
    registered.set_##prefix##N0((matmul).n0);                                                                         \
    registered.set_##prefix##MLoop((matmul).mLoop);                                                                   \
    registered.set_##prefix##KLoop((matmul).kLoop);                                                                   \
    registered.set_##prefix##NLoop((matmul).nLoop);                                                                   \
    registered.set_##prefix##CoreLoop((matmul).coreLoop);                                                             \
    registered.set_##prefix##SwizzleCount((matmul).swizzleCount);                                                     \
    registered.set_##prefix##SwizzleDirect((matmul).swizzleDirect);                                                   \
    registered.set_##prefix##EnShuffleK((matmul).enShuffleK);                                                         \
    registered.set_##prefix##BlockDim((matmul).blockDim);                                                             \
    registered.set_##prefix##EnLoadAllAmat((matmul).enLoadAllAmat);                                                   \
    registered.set_##prefix##B0matPingPongBufferLen((matmul).b0matPingPongBufferLen)

    COPY_FIELD(tilingKey);
    COPY_FIELD(userWorkspaceSize);
    COPY_FIELD(s1Offset);
    COPY_FIELD(s2Offset);
    COPY_FIELD(s3Offset);
    COPY_FIELD(s4Offset);
    ...
    COPY_FIELD(numCore);
    COPY_FIELD(n);
    COPY_FIELD(perTaskNum);
    COPY_FIELD(resTaskNum);

    COPY_MATMUL(mm1, source.mm1);
    COPY_MATMUL(mm2, source.mm2);
    COPY_MATMUL(mm3, source.mm3);

    COPY_FIELD(rmsNumCore1);
    COPY_FIELD(rmsNumCol1);
    COPY_FIELD(rmsNumRow1);
    COPY_FIELD(rmsQuantMin1);
    COPY_FIELD(rmsNumCore2);
    COPY_FIELD(rmsNumCol2);
    COPY_FIELD(rmsNumRow2);
    COPY_FIELD(rmsQuantMin2);
    COPY_FIELD(hiddenSizeQ);
    COPY_FIELD(headNumQ);
    COPY_FIELD(headDim);
    COPY_FIELD(concatSize);
    COPY_FIELD(rotaryCoeff);
    COPY_FIELD(ntokens);
    COPY_FIELD(realCore);
    COPY_FIELD(nlCoreRun);
    COPY_FIELD(lCoreRun);
    COPY_FIELD(maxNPerLoopForUb);
    COPY_FIELD(preCoreLoopTime);
    COPY_FIELD(preCoreLoopNLast);
    COPY_FIELD(lastCoreLoopTime);
    COPY_FIELD(lastCoreLoopNLast);
    COPY_FIELD(esqFrontCore);
    COPY_FIELD(esqTailCore);
    COPY_FIELD(esqFrontCoreBatch);
    COPY_FIELD(esqTailCoreBatch);
    COPY_FIELD(esqHeadNum);
    COPY_FIELD(esqColNum);
    COPY_FIELD(esqUbHeadLoop);
    COPY_FIELD(esqHeadPerLoop);
    COPY_FIELD(esqHeadTail);
    COPY_FIELD(esqColLoop);
    COPY_FIELD(esqColTail);
    COPY_FIELD(hiddenStateDim);
    COPY_FIELD(isWeightQuantized);
    COPY_FIELD(enableRope);
    COPY_FIELD(mm1OutSize);
    COPY_FIELD(splitSizeOne);
    COPY_FIELD(splitSizeTwo);
    COPY_FIELD(splitRmsNormSizeOne);
    COPY_FIELD(splitRmsNormSizeTwo);
    COPY_FIELD(ropeSplitSizeOne);
    COPY_FIELD(ropeSplitSizeTwo);
    COPY_FIELD(hiddenStrideRope);
    COPY_FIELD(qkNopeHeadDim);
    COPY_FIELD(avgFactor);
    COPY_FIELD(kvCacheBlockSize);
    COPY_FIELD(kvCacheStride0);
    COPY_FIELD(kvCacheRopeStride0);

#undef COPY_MATMUL
#undef COPY_FIELD
}

std::array<uint8_t, sizeof(MlaTilingData)> MakeNormalizedDeviceWireData(const MlaTilingData &source)
{
    std::array<uint8_t, sizeof(MlaTilingData)> bytes{};
    std::memcpy(bytes.data(), &source, bytes.size());

    constexpr size_t firstPaddingBegin = offsetof(MlaTilingData, tilingKey) + sizeof(source.tilingKey);
    constexpr size_t firstPaddingEnd = offsetof(MlaTilingData, userWorkspaceSize);
    constexpr size_t finalPaddingBegin = offsetof(MlaTilingData, avgFactor) + sizeof(source.avgFactor);
    constexpr size_t finalPaddingEnd = offsetof(MlaTilingData, kvCacheBlockSize);
    static_assert(firstPaddingBegin == 4 && firstPaddingEnd == 8, "unexpected MLA leading padding layout");
    static_assert(finalPaddingBegin == 460 && finalPaddingEnd == 464, "unexpected MLA final padding layout");
    std::memset(bytes.data() + firstPaddingBegin, 0, firstPaddingEnd - firstPaddingBegin);
    std::memset(bytes.data() + finalPaddingBegin, 0, finalPaddingEnd - finalPaddingBegin);
    return bytes;
}
...
ge::graphStatus ParseShapesAndAttrs(gert::TilingContext *context, mlapo::OpParam &opParam)
{
    ...
    OP_CHECK_IF(!IsPositiveU32(tokenNum) || !IsPositiveU32(hiddenStateDim) || !IsPositiveU32(headNum) ||
                ...
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(tokenNum > MAX_TOKEN_NUM,
                OP_LOGE(context, "token count must be in [1, %ld], got %ld.", MAX_TOKEN_NUM, tokenNum),
                return ge::GRAPH_FAILED);
...
ge::graphStatus TilingMlaPreprocess(gert::TilingContext *context)
{
    ...
    MlaPreprocessTilingData registeredTilingData;
    PopulateRegisteredTilingData(tilingData, registeredTilingData);
    auto *rawTilingData = context->GetRawTilingData();
    OP_CHECK_NULL_WITH_CONTEXT(context, rawTilingData);
    const size_t registeredDataSize = registeredTilingData.GetDataSize();
    OP_CHECK_IF(registeredDataSize != sizeof(MlaTilingData),
                OP_LOGE(context, "registered tiling ABI size %zu does not match kernel ABI size %zu.",
                        registeredDataSize, sizeof(MlaTilingData)),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(rawTilingData->GetCapacity() < registeredDataSize,
                OP_LOGE(context, "tiling buffer capacity %zu is smaller than %zu.", rawTilingData->GetCapacity(),
                        registeredDataSize),
                return ge::GRAPH_FAILED);
    registeredTilingData.SaveToBuffer(rawTilingData->GetData(), rawTilingData->GetCapacity());
    const auto *registeredBytes = static_cast<const uint8_t *>(rawTilingData->GetData());
    const auto deviceBytes = MakeNormalizedDeviceWireData(tilingData);
    size_t mismatchOffset = 0;
    while (mismatchOffset < registeredDataSize && registeredBytes[mismatchOffset] == deviceBytes[mismatchOffset]) {
        ++mismatchOffset;
    }
    OP_CHECK_IF(mismatchOffset != registeredDataSize,
                OP_LOGE(context,
                        "registered Host Tiling serialization differs from the device wire layout at byte %zu "
                        "(registered=0x%02x, device=0x%02x).",
                        mismatchOffset, static_cast<unsigned int>(registeredBytes[mismatchOffset]),
                        static_cast<unsigned int>(deviceBytes[mismatchOffset])),
                return ge::GRAPH_FAILED);
    rawTilingData->SetDataSize(registeredDataSize);
