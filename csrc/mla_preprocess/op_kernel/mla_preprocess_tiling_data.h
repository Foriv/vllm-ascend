...
struct MlaTilingData {
    ...
    uint32_t mm1OutSize{2112};
    uint32_t splitSizeOne{576};
    uint32_t splitSizeTwo{1536};
    uint32_t splitRmsNormSizeOne{512};
    uint32_t splitRmsNormSizeTwo{64};
    uint32_t ropeSplitSizeOne{64};
    uint32_t ropeSplitSizeTwo{128};
    uint32_t hiddenStrideRope{192};
    uint32_t qkNopeHeadDim{128};
    float avgFactor{0.000651041666f};
...
static_assert(sizeof(PpMatmulTilingData) == 17 * sizeof(uint32_t),
              "MLA matmul tiling ABI must remain 68 bytes");
static_assert(sizeof(MlaTilingData) == 488,
              "MLA device tiling ABI changed; update the registered Host Tiling layout together");
